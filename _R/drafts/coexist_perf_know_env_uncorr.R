...
dir.create(here("outputs/m3"), recursive=TRUE)
...
for (i in 1:n_axis) {
  seed <- 1234
  sites[,i] <- runif(nsite, 0, 1)
  env[[i]] <-  matrix(sites[,i], nrow=nsite_side, ncol=nsite_side, byrow=TRUE)
}
...
save(sites, file = here::here("outputs", "m3", "sites.RData"))
save(env, file = here::here("outputs", "m3", "env.RData"))
...
# Habitat frequency
png(file=here("outputs", "m3", "hab_freq_1.png"),
    width=fig_width, height=fig_width, units="cm", res=300)
hist(env[[1]], main="", xlab="Environment var1")    
dev.off()

png(file=here("outputs", "m3", "hab_freq_2.png"),
    width=fig_width, height=fig_width, units="cm", res=300)
hist(env[[2]], main="", xlab="Environment var2")    
dev.off()

png(file=here("outputs", "m3", "hab_freq_3.png"),
    width=fig_width, height=fig_width, units="cm", res=300)
...
png(file=here("outputs", "m3", "species_niche.png"),
    width=fig_width, height=fig_width, units="cm", res=300)
par(mar=c(1,1,2,2))
...
png(file=here("outputs", "m3", "hist_mortality.png"),
    width=fig_width, height=fig_width, units="cm", res=300)
hist(mortality_E_Sp)
dev.off()
# Mortality probability function
png(file=here::here("outputs", "m3", "function_mort_proba.png"),
...
save(sp_hab_freq, file = here::here("outputs", "m3", "sp_hab_freq.RData"))
...
for (r in 1:nrep) {
  ...
  #hist(sp)
  community_start_m3 <- matrix(sp, nrow=nsite_side, ncol=nsite_side, byrow=TRUE)
  if (r==1) {
    png(file=here("outputs", "m3", "community_start.png"),
        width=fig_width, height=fig_width, units="cm", res=300)
    par(bty = "n")
    plot(raster(community_start_m3), main="Species - Start", zlim=c(0, nsp),
         col=c("black",  viridis(nsp)), legend=FALSE, cex.main=2, cex.axis=1.5)
    dev.off()
  }
  community <- community_start_m3
  ...
  #CGT 17/06/2021
  #To keep the abundance matrixes in order to infer alpha matrix
  Abundances_m3[[r]] <- abund
  
} # End nrep

save(Abundances_m3, file = here::here("outputs", "m3", "Abundances_m3.RData"))
save(sp_rich, file=here::here("outputs", "m3", "sp_rich_m3.RData"))
save(rank_sp, file=here::here("outputs", "m3", "rank_sp_m3.RData"))
...
save(sp_rich_final, file=here::here("outputs", "m3", "Species_richness_m3.RData"))

#CGT 16/06/2021
# ---------------------------------------------
# Shannon index and Shannon equitability index
# ---------------------------------------------
save(Shannon, file = here::here("outputs", "m3", "Shannon_m3.RData"))
Equitability <- Shannon/log(as.numeric(sp_rich[ngen,]))
save(Equitability, file = here::here("outputs", "m3", "Equitability_m3.RData"))
...
save(Spearman, file = here::here("outputs", "m3", "Spearman_m3.RData"))
...
p <- ggplot(data=Df_rank_sp, aes(x=rep, y=rank_sp)) +
  ...
  theme(legend.position = "none",
        text = element_text(size = 20))
ggsave(p, filename=here("outputs", "m3", "species_rank_with_repetitions.png"),
       width=fig_width, height=fig_width/2, units="cm", dpi=300)
...
# Plot
png(file=here("outputs", "m3", "spatial_comp_env_sp.png"), 
    width=fig_width, height=fig_width, units="cm", res=300)
par(mfrow=c(2,2), bty = "n")
plot(raster(community_start_m3), main="Species - Start", zlim=c(0, nsp),
...
p <- ggplot(data=df_sp_sel, aes(x=Env, y=Perf)) +
  ...
  ylab("Performance")
ggsave(p, filename=here("outputs", "m3", "infering_species_niche.png"),
       width=fig_width, height=fig_width, units="cm", dpi=300)
...
save(lm_fit, file = here::here("outputs", "m3","lm_fit.RData"))
...
V_intra <- df_perf %>%
  ...
  group_by(Species) %>%
  summarise(V=var(res))
save(V_intra, file = here::here("outputs", "m3", "V_intra.RData"))
...
ggsave(p, filename=here("outputs", "m3", "intraspecific_variance.png"),
       width=fig_width, height=fig_width, units="cm", dpi=300)
...
p <- ggplot(df_perf_IV, aes(x = X, y = Perf, colour = as.factor(Species))) +
  ...
  labs(x = "Performance at mean environment variable X1",
       y = "Density")
ggsave(p, filename=here("outputs", "m3", "Perf_overlap_IV.png"),
       width=fig_width, height=fig_width/2, units="cm", dpi=300)
...
#
load(here::here("outputs", "m3", "V_intra.RData"))
load(here::here("outputs", "m3", "sp_hab_freq.RData"))
...
load(here::here("outputs", "m3", "Abundances_m3.RData"))
for (sp in 1:nsp) {
  if (Abundances_m3[[1]][nrow(Abundances_m3[[1]]), sp] == 0) {
    V_intra_hab_freq$Pres_m3[sp] <- 0
  }
  else V_intra_hab_freq$Pres_m3[sp] <- 1
}
...
for (k in 1:nrow(V_intra_hab_freq)){
  if (V_intra_hab_freq$Pres_m3[k]==1 && V_intra_hab_freq$Pres_m1[k]==1){
    V_intra_hab_freq$Code_presence[k] <- 3
  }
  if (V_intra_hab_freq$Pres_m3[k]==0 && V_intra_hab_freq$Pres_m1[k]==0){
    V_intra_hab_freq$Code_presence[k] <- 0
  }
  if (V_intra_hab_freq$Pres_m3[k]==1 && V_intra_hab_freq$Pres_m1[k]==0){
    V_intra_hab_freq$Code_presence[k] <- 1
  }
  if (V_intra_hab_freq$Pres_m3[k]==0 && V_intra_hab_freq$Pres_m1[k]==1){
...
ggplot2::ggplot(data = V_intra_hab_freq, aes(x = Hab_freq, y = IV, colour = Code_presence))+
  ...
  labs(title = "Relationship between the species suitable habitat frequency in m3, \n the IV on axis X1 and the species presence at the end of a simulation",
       x = "Suitable habitat frequency in m3",
...
abund_m3 <- Abundances_m3[[1]]
Time_disap_m3 <- c()
for (k in 1:ncol(abund_m3)){
  t <- abund_m3[,k]%>%purrr::detect_index(function(x) x == 0)
  Time_disap_m3 <- c(Time_disap_m3, t)
}
...
V_intra_hab_freq$Time_disap_m3 <- Time_disap_m3
V_intra_hab_freq$Time_disap_m1 <- Time_disap_m1

V_intra_hab_freq[which(V_intra_hab_freq$Time_disap_m3==0),]$Time_disap_m3 <- ngen
V_intra_hab_freq[which(V_intra_hab_freq$Time_disap_m1==0),]$Time_disap_m1 <- ngen

ggplot2::ggplot(data=V_intra_hab_freq, ggplot2::aes(x=IV, y=Time_disap_m3))+
  ggplot2::geom_point()+
  ggplot2::labs(x="IV", y="Time of species disappearing", title = "m3, 1 repetition, 1000 generations")

ggplot2::ggplot(data=V_intra_hab_freq, ggplot2::aes(x=Hab_freq, y=Time_disap_m3))+
  ggplot2::geom_point()+
  ggplot2::labs(x="Species habitat frequency", y="Time of species disappearing", title = "m3, 1 repetition, 1000 generations")
...
lm_fit_m3 <- lm(Time_disap_m3~IV+Hab_freq+IV*Hab_freq, data=V_intra_hab_freq[Time_disap_m3!=0,])
