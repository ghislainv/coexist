  ...
  for (r in 1:nrep) {
    ...
    for (g in 1:ngen) {
      ...
      if(n_mort!=0){
        ...
        } # end condition on abundance dependance
...
combi_models <- combn(unique(c(1:length(models))),2)
...
combi_models_jaccard <- as.data.frame(t(combi_models))
combi_models_jaccard$Jaccard <- mean_jaccard
colnames(combi_models_jaccard)[1:2] <- c("mod1", "mod2")

combi_models_percentage_similarity <- as.data.frame(t(combi_models))
...
p <- ggplot2::ggplot(data = combi_models_percentage_similarity, ggplot2::aes(x=mod1, y=mod2, fill=Percentage_similarity))+ 
  ...
  ggplot2::scale_fill_viridis_c()+
  ggplot2::coord_fixed()+
  ggplot2::labs(title="Percentage similarity index of the species abundances of the final community \n
                (mean of all repetitions)",
                x="Model 1",
                y = "Model 2")
